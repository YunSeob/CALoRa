...
class DSBlock(nn.Module):
    def __init__(self, cin, cout, k=(5,3), g=None):
        super().__init__()
        g = cin if g is None else g  # depthwise
...
class ColumnNetV2(nn.Module):
    ...
    def forward(self, x):                   # (B,1,128,660)
        f = self.stem(x)                    # (B,C,128,660)
        f = self.pool_h(f).squeeze(2)       # (B,C,660)
        return f
...
class TemporalTCN(nn.Module):
    def __init__(self, C=32):
        ...
        self.net = nn.Sequential(blk(1), blk(2), blk(4))
        self.head = nn.Conv1d(C, 1, 1)
    def forward(self, f):                    # (B,C,T)
        h = self.net(f)
        p = torch.sigmoid(self.head(h))      # (B,1,T)
        return p.squeeze(1)                  # (B,T)

class PreambleDetectorV2(nn.Module):
    def __init__(self, C=32, Ls=(264,)):
        super().__init__()
        self.col = ColumnNetV2(C)
        self.tem = TemporalTCN(C)
        self.register_buffer('Ls', torch.tensor(Ls, dtype=torch.int64))  # 여러 길이 지원
    @torch.no_grad()
    def _sliding_sum(self, p, L):
        # p: (B,T)  누적합으로 윈도우합 계산
        cs = torch.cumsum(p, dim=-1)                  # (B,T)
        # s[t] = sum_{t..t+L-1} p = cs[t+L-1] - cs[t-1]
        s = cs[:, L-1:] - F.pad(cs[:, :-L], (1,0), value=0)
        return s                                       # (B,T-L+1)
    def forward(self, x):
        f = self.col(x)                                # (B,C,T)
        p = self.tem(f)                                # (B,T)
        # 다중 L 처리: 각 L에 대해 슬라이딩 합 계산 후 최대값 채택
        sums = [self._sliding_sum(p, int(L)) for L in self.Ls]
        s = torch.stack([F.pad(si, (0, p.size(-1)-si.size(-1))) for si in sums], dim=1)  # (B,#L,T’)
        s_max, _ = torch.max(s, dim=1)                 # (B,T’)
        return p, s_max